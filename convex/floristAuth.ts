...
export const findFloristByEmail = query({
  args: { email: v.string() },
  returns: v.union(
    v.null(),
    v.object({
  ...
  handler: async (ctx, args) => {
    ...
    const florist = await ctx.db
      ...
      .first();
    return florist ? { _id: florist._id, email: florist.email ?? null } : null;
  },
});
...
export const createOrReuseOtp = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("floristOtps", {
      email,
      code,
      expiresAt: now + OTP_TTL_MS,
      createdAt: now,
...
export const sendOtpEmail = action({
  ...
  handler: async (ctx, args) => {
    ...
    const florist = await ctx.runQuery(api.floristAuth.findFloristByEmail, { email });
    if (!florist) {
      return {
        success: false,
        message:
          "Флориста з таким email не знайдено. Якщо ви подали заявку — дочекайтесь підтвердження адміністратора.",
      };
    }

    const otp = await ctx.runMutation(api.floristAuth.createOrReuseOtp, { email });
    if (!otp.shouldSend) {
    ...
    if (!RESEND_API_KEY) {
      console.log("[DEV] Florist OTP for", email, ":", otp.code);
      ...
      const response = await (globalThis as any).fetch(
        "https://api.resend.com/emails",
        {
          ...
          body: JSON.stringify({
            from: OTP_FROM,
            to: email,
            subject: "Ваш код входу (флорист) - Blomm Daya",
            html: `
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
<h2 style="color: #C084FC;">Blomm Daya</h2>
<p>Ваш код для входу (флорист):</p>
...
`,
      ...
      if (!response.ok) {
        ...
        if (isResendTestModeRestriction(errorText)) {
          ...
          console.warn("Resend test-mode restriction (florist):", {
            to: email,
            fromDomain,
          });
          console.log("[DEV] Florist OTP for", email, ":", otp.code);
...
export const verifyOtp = action({
  ...
  returns: v.object({
    ...
    floristId: v.union(v.null(), v.string()),
  }),
  handler: async (ctx, args) => {
    const email = args.email.trim().toLowerCase();
    const code = args.code.trim();

    return await ctx.runMutation(api.floristAuth.verifyOtpMutation, {
      email,
      code,
    });
  },
...
export const verifyOtpMutation = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!florist) {
      return {
        success: false,
        message: "Флориста не знайдено",
        token: null,
        floristId: null,
      };
    }

    const token = `florist_${Math.random().toString(36).slice(2)}_${Date.now()}`;
    await ctx.db.insert("floristSessions", {
...
export const getCurrentFlorist = query({
  args: {
    token: v.string(),
  },
  returns: v.union(
  ...
  handler: async (ctx, args) => {
    ...
    const florist = await ctx.db.get(session.floristId);
